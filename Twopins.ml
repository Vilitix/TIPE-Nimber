...
let search_pattern board:int array list = (*renvoi une liste de tous les jeu *)
  let n = Array.length board in 
  let rec aux board l i:(int array list) = (*algorithme de recherche dans un texte pas intéreNim_func.SSant pour mot de taille 3 dans un alphabet de taille 3 *)
    if i < (n-3) then 
    match  board.(i),board.(i+1),board.(i+2) with
    |2,1,2-> 
      begin 
        let l1,l2 = decomp_212 board (i+1) in 
        aux board (l1::l) (i+1)
      end
    |x,y,z->  aux board l (i+1)
    else l
  in
  aux board [] 0;;
...
let rec nim_naif board =
  let set = ref Nim_func.SS.empty in
  let n = Array.length board in
  for i = 0 to (n - 1) do
    if (i = 0) && (board.(0) = 1) then
