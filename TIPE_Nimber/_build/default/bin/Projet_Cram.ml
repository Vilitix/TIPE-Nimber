    ...
    for j = 0 to p-1 do 
      if (tab.(i).(j)) then 
        ...
        (if (k1<n)&&(l1<p)&&(tab.(k1).(l1)) then 
          uf := Unionfind.union !uf (mat_to_tab (i+1) (j+1) (p+2)) (mat_to_tab (k1+1) (l1+1) (p+2)));
        (if (k2<n)&&(l2<p)&&(tab.(k2).(l2)) then
          uf := Unionfind.union !uf (mat_to_tab (i+1) (j+1) (p+2)) (mat_to_tab (k2+1) (l2+1) (p+2)));
        end
    done;
  done;
  uf
