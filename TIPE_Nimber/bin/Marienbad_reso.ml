...
let affiche_tab (tab:int array):unit =
...
let somme_tab (tab:int array):int = (*pour la taille maximum du tableau de nimber *)
let x = ref 0 in 
...
let nimber_tab (tab:int array):int =
  let nb = ref 0 in
  for i = 0 to Array.length tab - 1 do
...
let choisir_coup_parfait (tab:int array):int*int =
...
let is_tab_null (tab:int array):bool =
...
let recopie_label_nimber (nb:int) (channel:out_channel) (tab:string array):unit = 
  (*permet d'ajouter des noeuds et de les nommer*)
for i=0 to ((Array.length tab ) -1) do 
...
let graphe_end (file:int Queue.t):unit = (*graphe finale de l'affichage de tous les coups de la partie*)
  let channel = open_out "/home/arthur/Desktop/TIPE/sortie.txt" in
  Printf.fprintf channel "digraph \"Déroulement de la partie\" {
    fontname=\"Helvetica,Arial,sans-serif\"
    node [fontname=\"Helvetica,Arial,sans-serif\"]
    edge [fontname=\"Helvetica,Arial,sans-serif\"]
    nodesep = 1;\n\n";
...
let marienbad (tab:int array):unit =
  affiche_tab tab;
  ...
  (*affiche le graphe des coups possibles et leur nimber*)
  while not (is_tab_null tab) do
    ...
    if is_tab_null tab then joueur_gagne := true
      ...
      Printf.printf "\nL'ordinateur a enlevé %d objet(s) dans le tas numéro %d\n" nb num_tas;
      tab.(num_tas) <- tab.(num_tas) - nb;
      affiche_tab tab;
  done;
  graphe_end file_partie;
  (*graphe de tous les coups joués par ordi et joueur selon leur nimber*)
  if !joueur_gagne then print_string "Bravo, tu as gagné!" else print_string "Dommage, l'ordinateur a gagné."
