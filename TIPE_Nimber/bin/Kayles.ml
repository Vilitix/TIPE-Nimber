...
let mat_kayles = Array.make 100 None;; (*on place dans ce tableau mat_kayles[i] = nimber du jeu de Kayles avec n colonnes de 1 quille *)
mat_kayles.(0) <- Some 0;;
mat_kayles.(1) <- Some 1;;
;;

let rec nim_cas_general n = (*Nimber de K_{n} si tout calculer O(nlog(n^2)) sinon somme de n*cela majoré par nlogn^2 d ou n^2log(n^2)*)
...
let nim board = (*O(n)*)
	...
	for i = 0 to (n-1) do 
		...
		(*Optimal nimber add en O(1), et nim_cas_general supposé calculé en 0(1)*)
			j:= i+1
		done;
